        ...
        stage('SAST-Semgrep') {
            ...
            steps {
                script{
                    ...
                    try {
                        sh 'semgrep scan --json --error > semgrep.json' // con el flag --json-output generamos un reporte en formato json y con --error hacemos que semgrep devuelva un código de salida distinto de 0 si encuentra alguna vulnerabilidad
                    }
                    catch (err) {                                        
